...
#* 可拖拽网格对象
@onready var draggable_grid: DraggableGrid = $VBoxContainer/DraggableGrid
...
func _on_slot_added(added_slot: DraggableSlot, slot_position: Vector2i) -> void:
	update_info()
	print("槽位已添加: 位置 ", slot_position, " 大小 ", added_slot.slot_size)

#* 处理槽位移动事件
#* @param moved_slot 被移动的槽位对象
#* @param old_slot_position 移动前的位置（格子坐标）
#* @param new_slot_position 移动后的位置（格子坐标）
func _on_slot_moved(moved_slot: DraggableSlot, old_slot_position: Vector2i, new_slot_position: Vector2i) -> void:
	update_info()
	print("槽位已移动: ", old_slot_position, " -> ", new_slot_position)
...
func update_info() -> void:
	if info_label and draggable_grid:
		...
		info_label.text = "当前槽位数: %d\n操作说明:\n• 左键点击并拖拽槽位移动\n• 点击按钮添加新槽位" % slot_count
