...
func world_to_grid(world_pos: Vector2) -> Vector2i:
	...
	var grid_x = floor(local_pos.x / TILE_WIDTH)
	var grid_y = floor(local_pos.y / TILE_HEIGHT)
	#* 限制格子范围（可选，避免负数/超出地图）
...
func grid_to_world_center(grid_pos: Vector2i) -> Vector2:
	var world_pos = grid_origin + Vector2(
		grid_pos.x * TILE_WIDTH + TILE_WIDTH / 2.0,
		grid_pos.y * TILE_HEIGHT + TILE_HEIGHT / 2.0
	)
	return world_pos
